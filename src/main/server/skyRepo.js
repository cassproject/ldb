...
global.repoAutoDetect = function() {
    if ((process.env.CASS_LOOPBACK || process.env.LOOPBACK) != null) {
        repo.init((process.env.CASS_LOOPBACK || process.env.LOOPBACK), function() {
            global.auditLogger.report(global.auditLogger.LogCategory.SYSTEM, global.auditLogger.Severity.INFO, 'LdbRepoInit', EcObject.keys(EcRemoteLinkedData.forwardingTable).length + ' records now in forwarding table.');
        }, (error) => {
            global.auditLogger.report(global.auditLogger.LogCategory.SYSTEM, global.auditLogger.Severity.ERROR, 'LdbRepoInitError', error);
        });
    } else {
        repo.autoDetectRepository();
    }

    global.auditLogger.report(global.auditLogger.LogCategory.SYSTEM, global.auditLogger.Severity.INFO, 'LdbRepoAutoDetect',
...
const signatureSheet = async function() {
    ...
    for (let i = 0; i < sigSheet.length; i++) {
        ...
        global.auditLogger.report(global.auditLogger.LogCategory.SYSTEM, global.auditLogger.Severity.INFO, 'LdbIdentity', pk.fingerprint());
        ...
        if ((rld.reader != null && rld.reader.length != 0) || isEncryptedType(rld)) {
            ...
            if (this.ctx.req.eim != null) {
                try {
                    ...
                    global.auditLogger.report(global.auditLogger.LogCategory.SYSTEM, global.auditLogger.Severity.ERROR, 'LdbDecryptError', 'We couldn\'t decrypt it, hope the client has better luck! -- ' + msg);
                }
            }
        }
...
const pingWithTime = function() {
    return JSON.stringify({
        ...
        ssoLogin: this.ctx.req.oidc ? (process.env.CASS_OIDC_BASE_URL || process.env.OIDC_BASE_URL || 'http://localhost/') + 'api/login' : undefined,
        ssoLogout: this.ctx.req.oidc ? (process.env.CASS_OIDC_BASE_URL || process.env.OIDC_BASE_URL || 'http://localhost/') + 'api/logout' : undefined,
        banner: (process.env.CASS_BANNER_MESSAGE || process.env.BANNER_MESSAGE) ? {
            message: process.env.CASS_BANNER_MESSAGE || process.env.BANNER_MESSAGE, // string
            color: process.env.CASS_BANNER_TEXT_COLOR || process.env.BANNER_TEXT_COLOR, // valid css color value
            background: process.env.CASS_BANNER_BACKGROUND_COLOR || process.env.BANNER_BACKGROUND_COLOR, // valid css color value
        } : undefined,
 ...
 *                   type: string
 *                   required: false
 *                   description: When logged in with OIDC (or similar), the URL of the login redirect page, using OIDC_BASE_URL environment variable for the endpoint.
 *                   example: http://localhost/api/login
 *                 ssoLogout:
 *                   type: string
 *                   required: false
 *                   description: When logged in with OIDC (or similar), the URL of the logout redirect page, using OIDC_BASE_URL environment variable for the endpoint.
 *                   example: http://localhost/api/logout
 *                 banner:
 *                   type: object
 *                   required: false
 *                   description: If specified in BANNER_MESSAGE, BANNER_TEXT_COLOR, BANNER_BACKGROUND_COLOR environment variables communicated from the server.
 *                   properties:
 *                     message:
 *                       type: string
 *                       required: true
 *                       description: Banner text as specified by BANNER_MESSAGE environment variable.
 *                       example: <Security markings>
 *                     color:
 *                       type: string
 *                       required: true
 *                       description: CSS text color as specified by BANNER_TEXT_COLOR environment variable.
 *                       example: red
 *                     background:
 *                       type: string
 *                       required: true
 *                       description: CSS background color as specified by BANNER_BACKGROUND_COLOR environment variable.
 *                       example: yellow
 *                 motd:
 *                   type: object
 *                   required: false
