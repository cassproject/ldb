...
LDB is built upon [Open Data](http://opendefinition.org/od/2.1/en/) principles combined with a security and permissioning system built on Public Key Infrastructure.

Actors in the LDB ecosystem can be users, other systems, or applications acting on a user’s behalf. All of these actors use the same tool to identify themselves: A RSA public/private keypair.
...
Encodings of Private Keys include the Public Key and we denote by “**PPK**” (Public/Private Keypair). “**PK**” is used to denote just the Public Key. LDB refers to the PPK as an ‘*identity*’ and its PK portion is an ‘*identifier*’.

## Owners, Readers, Actions and Signature Sheets

All permissioned objects in LDB have an ```@owner``` field that contains the identifier of the identity that owns it. All compliant LDB repositories will validate the ```@owner``` field against a “Signature Sheet” before allowing a privileged action to occur.
...
![Visual](/v1.5/identities-and-login-1.png)

This is also possible with PKI.

![Visual](/v1.5/identities-and-login-2.png)
...
LDB comes with a no-knowledge keystore (called the **Remote Identity Manager**) that allows users to operate LDB with a username and password. It is typically not reasonable to require individuals to manage their own encryption keys. It does not have login recovery capabilities, and is generally not recommended.

LDB is compatible with any login or SSO system that can store or provide a place to hold a PPK. The individual’s keys  should be retrievable in case they wish to use other tools to operate on or view their data.
...
An EcPpk is a public/private keypair helper object that wraps an actor identity in LDB. The object can transform and serialize the key contained within.
...
An EcPk is a public key helper object that represents an actor identifier in LDB. The object can transform and serialize the key contained within.
